...
def find_divisible(max_number, divisor) -> list:
    dividable: list = []

    for i in range(1, max_number + 1):
        if i % divisor == 0:
            dividable.append(i)  # ZDE NAPIŠTE VÁŠ KÓD
    return dividable
...
if __name__ == "__main__":
    ...
    print(
        f"Numbers divisible by {divisor} less than or equal to {max_number}: {result}"
    )
