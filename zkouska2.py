...
def fetch_weather_data(city):
    ...
    if response.status_code == 404:
        ...
        temperature = round(response.json()["main"]["temp"] - 273.15, 2)
        return temperature
...
def test_fetch_weather_data():
    mock_response = {"main": {"temp": 293.15}}  # Teplota v Kelvinech
    with patch("requests.get") as mock_get:
        mock_get.return_value = MagicMock(
            ok=True, status_code=200, json=MagicMock(return_value=mock_response)
        )
